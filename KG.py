...
#neo4j
driver = GraphDatabase.driver("bolt://localhost:7687",auth=("neo4j","mm430422"))
...
def GetRelationPathsSingle(entity):
    ...
    for nametype in nametypes:
        ...
        props=session.run(cql_2, name=entity)
...
def GetRelationPaths(entity):
    ...
    for nametype in nametypes:
        ...
        resprop=session.run(cql_2,name=entity)
        for record1 in res:
...
def GetRelationNum(entity):
    ...
    for nametype in nametypes:
        ...
        res = session.run(cql1,name=entity)
        ans = 0
        for record in res:
            ans = record.values()[0]
        relcounts+=ans
