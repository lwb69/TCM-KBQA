import pickle
from utils import  ,features_from_two_sequences
from KG import GetRelationPathsSingle, GetRelationNum,GetRelationPaths
import codecs as cs
import time
...
class EntityExtractor(object):
    def __init__(self):
        self.mention2entity_dic = pickle.load(open('../data/mention2entity_dic_cm4.pkl', 'rb'))
        allseq_dict = {}
        with open('../KG/allseg.txt', 'r', encoding='utf-8') as fp:
            for line in fp:
                if line:
                    ename = line.strip('\n')
                    allseq_dict[ename] = 1
            ...
            if mention in self.mention2entity_dic:  # 如果它有对应的实体
                for entity in self.mention2entity_dic[mention]:
                    ...
                    if entity in self.entity2hop_dic:
                        ...
                        relations = GetRelationPathsSingle(entity)
