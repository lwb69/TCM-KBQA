...
class KGapi:
    ...
    def GetRelationPathsSingle(self,entity):
        ...
        for nametype in self.nametypes:
            ...
            # if nametype == '方剂名称':
            #     cql_1 = "match (a:`美容方`)-[r1]-() where a.{}=$name return DISTINCT type(r1)".format(nametype)
            #     cql_2 = "match (a:`美容方`)where a.{}=$name return properties(a)".format(nametype)
    ...
    def GetRelationPaths(self,entity):
        ...
        for nametype in self.nametypes:
            ...
            # if nametype == '方剂名称':
            #     cql_1 = "match (a:`美容方`)-[r1]-()-[r2]-() where a.{}=$name return DISTINCT type(r1),type(r2)".format(nametype)
            #     cql_2 = "match (a:`美容方`)-[r1]-(b)where a.{}=$name return type(r1),properties(b)".format(nametype)


            rpaths1 = []
            path2props= []
            res = self.session.run(cql_1,name=entity)
    ...
    def Gettypeofentity(self,entity):
        for nametype in self.nametypes:
            cql = "match (a)where a.{}=$name return labels(a)".format(nametype)
            props = self.session.run(cql, name=entity)
            props_list=[]
            for record2 in props:
                for prop in record2['labels(a)']:
                    props_list.append(prop)
            if len(props_list)==0:
                continue
            else:
