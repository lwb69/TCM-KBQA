...
from qa.system.QA_system.similarity_bert import BertModel_Simmer, DataPrecessForSentence
import torch
import codecs as cs
import random
...
class PathExtractor(object):
    def __init__(self):
        ...
        # 加载一些缓存
        try:
            self.entity2relations_dic = pickle.load(open('qa/system/data/entity2relation_dic.pkl', 'rb'))
        except:
            self.entity2relations_dic = {}
        self.fp = cs.open('qa/system/data/record/path_extrator_' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                                 time.localtime(time.time())) + '.txt',
                          'w')
        # 加载基于Robert的微调过的文本匹配模型
        self.simmer = BertModel_Simmer().to(torch.device("cuda"))
        self.simmer.load_state_dict(torch.load("qa/system/QA_system/RoBerta/models/fine_tune_best.pth.tar")["model"])
        self.simmer.eval()
        self.KGapi = KGapi()
    ...
    def extract_tuples(self, candidate_entitys, question):
        ...
        for entity in entity_list:
            ...
            if entity in self.entity2relations_dic:
                ...
                relations1hop = self.KGapi.GetRelationPathsSingle(entity)
                # relations2hop = self.KGapi.GetRelationPaths(entity)
                relsdic_list = []
                relsdic2_list = []
                ...
                print('relsdic_list:',relsdic_list)
                print('relsdic2_list',relsdic2_list)
        ...
        for i in range(num_batches):
            ...
            with torch.no_grad():
                batch_seqs, batch_seq_masks, batch_seq_segments = inputs_encode[0][begin:end], inputs_encode[1][begin:end], \
                                                                  inputs_encode[2][begin:end]
                seqs, masks, segments = batch_seqs.to(torch.device("cuda")), batch_seq_masks.to(
                    torch.device("cuda")), batch_seq_segments.to(torch.device("cuda"))
